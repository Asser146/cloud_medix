...
class LabCubit extends Cubit<LabState> {
  ...
  List<String> status = ['REQUESTED', 'INITIATED', 'COMPLETED'];
  ...
  Future<void> getResult(String testId) async {
    emit(LabResultLoading(tests));
    String? id = await storage.read(key: "id");
    if (id == null) {
      emit(const LabError("User ID is missing Error"));
      return;
    } else {
      ApiResponse response = await repo.getTestResult(id, testId);
      if (response.status == 200 && response.data != null) {
        emit(LabResultLoaded(response.data, tests));
      } else {
        emit(LabError(response.error ?? "No medical record available."));
      }
    }
